...
/**
 * Micro-Influencer Hub — Blush themed Landing
 * Full page ready to paste in app/page.tsx (Next.js App Router)
 */
...
export default function LandingPage() {
  ...
  // Ring color helper (Tailwind)
  const ringStyle = useMemo(
    () => ({ ["--tw-ring-color" as any]: theme.accent } as React.CSSProperties),
    [theme]
  );

  // CTA color
  const primaryBtn = useMemo(
    () => ({ backgroundColor: theme.primary, color: theme.name === "Noir" ? "#f7f7f7" : "white" }),
    [theme]
  );

  // All images (editable from the UI)
    ...
    <div className="min-h-screen bg-white text-zinc-900">
          ...
          <CardContent className="p-4 flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <div className="text-zinc-600">
                Blush palette selected. Replace the image URLs below to match your niche. Prompts are listed at the bottom.
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="lg:col-span-3 grid md:grid-cols-2 xl:grid-cols-3 gap-3">
          ...
          {Object.entries(PRESETS).map(([key, p]) => (
            <button
              ...
              className={`border rounded-xl px-3 py-2 text-sm hover:shadow text-left ${
                presetKey === key ? "ring-2 ring-offset-2" : ""
              }`}
              style={{ borderColor: p.primary }}
              title={p.name}
            >
              <span className="inline-block w-4 h-4 rounded-full mr-2 align-middle" style={{ backgroundColor: p.primary }} />
              <span className="align-middle">{p.name}</span>
            </button>
          ))}
        </div>

        {/* Inputs for all images so setters are used (no ESLint warnings) */}
        <div className="lg:col-span-3 grid md:grid-cols-2 xl:grid-cols-3 gap-3">
          ...
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Mood 1 URL" value={mood1Url} onChange={(e) => setMood1Url(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Mood 2 URL" value={mood2Url} onChange={(e) => setMood2Url(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Mood 3 URL" value={mood3Url} onChange={(e) => setMood3Url(e.target.value)} />

          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Why image URL" value={whyUrl} onChange={(e) => setWhyUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Quality image URL" value={qualityUrl} onChange={(e) => setQualityUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Step 1 Post URL" value={stepPostUrl} onChange={(e) => setStepPostUrl(e.target.value)} />

          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Step 2 Boost URL" value={stepBoostUrl} onChange={(e) => setStepBoostUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Step 3 Analyze URL" value={stepAnalyzeUrl} onChange={(e) => setStepAnalyzeUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Step 4 Earn URL" value={stepEarnUrl} onChange={(e) => setStepEarnUrl(e.target.value)} />

          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Community Live URL" value={communityLiveUrl} onChange={(e) => setCommunityLiveUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Community Chat URL" value={communityChatUrl} onChange={(e) => setCommunityChatUrl(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="Community Best URL" value={communityBestUrl} onChange={(e) => setCommunityBestUrl(e.target.value)} />

          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="UGC 1 URL" value={ugc1Url} onChange={(e) => setUgc1Url(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="UGC 2 URL" value={ugc2Url} onChange={(e) => setUgc2Url(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="UGC 3 URL" value={ugc3Url} onChange={(e) => setUgc3Url(e.target.value)} />
          <input className="border rounded-xl px-3 py-2 text-sm" placeholder="UGC 4 URL" value={ugc4Url} onChange={(e) => setUgc4Url(e.target.value)} />
        </div>
      </div>
      ...
      <section className={`relative overflow-hidden bg-gradient-to-br ${theme.heroGradient}`}>
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-16 sm:py-24 grid md:grid-cols-2 gap-10 items-center">
          ...
          {/* Right: image collage + organizer card */}
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="space-y-4">
            <div className="grid grid-cols-5 gap-3">
              <div className="col-span-3 rounded-2xl overflow-hidden ring-4" style={ringStyle}>
                <img src={hero2Url} alt="Hero side" className="w-full h-48 object-cover" />
              </div>
              <div className="col-span-2 rounded-2xl overflow-hidden ring-4" style={ringStyle}>
            ...
            <Card className="backdrop-blur bg-white/70 border-0 shadow-xl rounded-2xl">
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  <img src={profileUrl} alt="Organizer" className="w-20 h-20 rounded-full object-cover ring-4" style={ringStyle} />
                  <div>
      ...
      <section className="max-w-6xl mx-auto px-4 -mt-6">
        <div className="grid md:grid-cols-3 gap-4">
          {[{ url: mood1Url, label: "Beauty" }, { url: mood2Url, label: "Fashion" }, { url: mood3Url, label: "Shopping" }].map((m, i) => (
            <div key={i} className="relative rounded-2xl overflow-hidden group ring-4" style={ringStyle}>
              <img src={m.url} alt={m.label} className="w-full h-48 md:h-56 object-cover group-hover:scale-[1.02] transition" />
              <div className="absolute bottom-3 left-3 bg-white/80 backdrop-blur px-3 py-1 rounded-full text-xs font-medium" style={{ color: theme.primary }}>
                {m.label}
              </div>
            </div>
          ))}
      ...
      <section className="max-w-6xl mx-auto px-4 py-14 grid md:grid-cols-2 gap-8">
        <Card className="rounded-2xl overflow-hidden">
          ...
          <CardContent className="p-6">
            ...
            <p className="text-zinc-700">
              Launching a new product (digital or physical) usually needs serious upfront cash (often €10k+).
              Instead, we co-create with multiple influencers, pooling audiences for higher success — with <strong>zero upfront cost</strong> to you.
            </p>
        ...
        <Card className="rounded-2xl overflow-hidden">
          ...
          <CardContent className="p-6">
            ...
            <p className="text-zinc-700">
              We don’t dropship low-quality items. We build <strong>premium, audience-driven</strong> products with trusted suppliers — strengthening your personal brand and customer trust.
            </p>
            <ul className="mt-4 space-y-2 text-sm">
      ...
      <section className="max-w-6xl mx-auto px-4 py-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[{u: ugc1Url}, {u: ugc2Url}, {u: ugc3Url}, {u: ugc4Url}].map((x, i) => (
            <div key={i} className="rounded-2xl overflow-hidden ring-4" style={ringStyle}>
              <img src={x.u} alt={`UGC ${i + 1}`} className="w-full h-60 object-cover" />
            </div>
          ))}
        </div>
      </section>
